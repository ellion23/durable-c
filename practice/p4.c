...
//Алгоритм метода.
//1. Определяем по графику приближённое значение корня x1.
//2. Находим значение функции и её производной в данной точке.
//3. Точка пересечения касательной в точке (x1, f'(x1)) равна x = x1 - f(x1) / f'(x1).
//4. Берём найденное значение за новое приближённое значение и повторяем шаги 2, 3, 4 пока не достигнем желаемой точности e.
//
